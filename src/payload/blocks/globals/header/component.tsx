...
export const HeaderBlock = async () => {
	...
	return (
		<nav className="bg-white shadow dark:bg-gray-800">
			<Container className="flex items-center justify-center gap-3 py-5 capitalize text-gray-500 dark:text-gray-300">
				<Link href="/" className="flex items-center justify-center">
					<div className="relative h-6 w-6 text-gray-800">
						<RenderImage
							imageAlt={`${serverURL}${logoAlt}`}
							imageSrc={`${serverURL}${logoUrl}`}
						/>
					</div>
				...
				<div>
					{header.nav.map((navLinkItem) => (
						<Link
							key={navLinkItem.link.url}
							href={
								navLinkItem.link.url !== null && navLinkItem.link.url !== undefined
									? navLinkItem.link.url
									: "/"
							}
